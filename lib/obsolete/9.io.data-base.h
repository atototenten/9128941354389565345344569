...
#define _DATABASE__TABLE__ROW_OFFSET(\
	...
	rowSize_,\
	inRowOffset_) \
	((inRowOffset_ + (rowid * rowSize_)) + tableRowsOffset_)
...
#define DATABASE__READ__TABLE__ROW(\
	...
	rowSize_,\
	buffer_) \
	const uu offset_ = _DATABASE__TABLE__ROW_OFFSET(tableRowsOffset_, rowid, rowSize_, 0);\
...
#define DATABASE__WRITE__TABLE__ROW__APPEND(\
	...
	const uu\
		rowid = rowCounter_++,\
		offset_ = _DATABASE__TABLE__ROW_OFFSET(tableRowsOffset_, rowid, rowSize_, 0);\
...
/*
	void
*/
#define DATABASE__WRITE__TABLE__ROW(result_, tableRowsOffset_, rowid, rowSize_, buffer_, count_, inRowOffset_) {\
	const uu offset_ = _DATABASE__TABLE__ROW_OFFSET(tableRowsOffset_, rowid, rowSize_, inRowOffset_);\
