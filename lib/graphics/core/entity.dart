part of "_.dart";

class entity__overlay___compo //
    implements
        entity___protocol {
  const entity__overlay___compo({
    required this.dispose__handle,
    required this.widget__build,
  });

  final dispose___procedure__format? dispose__handle;
  final widget__build__function__format widget__build;

  @override
  void dispose() {
    final dispose__handle_1 = dispose__handle;

    if (dispose__handle_1 == null) {
      return;
    }

    dispose__handle_1();
  }
}

extension entity__page___extension //
    on entity__page___protocol {
  entity__overlay___compo entity__overlay() {
    return entity__overlay___compo(
      dispose__handle: dispose,
      widget__build: this.widget__build,
    );
  }
}

abstract class entity__page___protocol //
    implements
        entity___protocol {
  widget___protocol //
      widget__build(final widget__building__context___compo context);
}

/*abstract class entity__component___protocol /*
instances must not ,be created in `widget::build`
  ,even if `.dispose` is set to be invoked ,through `widget__build.detach`
    ,because attach/detach are not called ,if tree is re-fresh-ed ,without altering
  ,no problem ,in event-emitted proc.s ,like `event__press__handle`
more efficient(run-time and run-memory-space) ,than closures ,due to dart-lang.'s design-choices
flagship(current) examples :
  - `list_ing__grow_able__entity` */ //
    <children__type extends Object> //
    implements
        entity___protocol {
  widget___protocol //
      widget__build /*
not named `build`
  because its vague(because its an internal-part ,of the widget)
    ,for a depo.(which contains data ,and g.u.i. is merely an optional part-of-it) */
      (
    final widget__building__context___compo context, {
    required covariant final children__type children,
  });
}*/

abstract class entity___protocol /*
`::dispose` should be called ,only once
derived `::dispose`s should begin with calling `super::dispose` */ //
    implements
        dispose___protocol {}
