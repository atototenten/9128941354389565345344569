...
inspired by `flutter/lib/src/material/app.dart`::`MaterialApp` */
{
  ...
  final init_ion__promise = ASYN__PROMISE<void>();
  {
    ...
    platform__init_().handle(
      (_) => _success__handle(),
      _failure__handle,
    );

    storage__init_().handle(
      (_) => _success__handle(),
      _failure__handle,
    );
...
called at last ,to allow ,the slow environments (like android) ,some extra time, for proper init. */
  ...
  runApp(
    Directionality(
      ...
      child: Localizations(
        locale: app__locale,
        delegates: app__localizations__delegates,
        child: app__widget(
...
un-"await"-ed to allow the app to normally function ,for non-denial cases */;
  ...
  app__theme__colors__ground__back__follow__system___ok = TRUE;

  app__run(
    init: (final init_ion__status) {
      return app__running__simple<entity__type>(
...
<entity__type extends entity__page___protocol> //
  ...
  app__running__simple({
    ...
    init_ion__status__asyn.handle(
      (_) {
        app__theme__colors__ground__back__contrast__dark___ok = environment__theme__colors__ground__back__contrast__dark___ok();
...
late BOOL app__theme__colors__ground__back__follow__system___ok; /*
...
class app__widget extends StatefulWidget {
  const app__widget({
  ...
  _app__widget__state createState() {
    return _app__widget__state();
  }
}

class _app__widget__state //
    extends
        State<app__widget> //
  ...
  void didChangePlatformBrightness() {
    ...
    if (app__theme__colors__ground__back__follow__system___ok.NOT) {
      return;
    }

    app__theme__colors__ground__back__contrast__dark___ok = environment__theme__colors__ground__back__contrast__dark___ok();

    app__theme__change__channel.event__dispatch();
  ...
  void didChangeAppLifecycleState(
    ...
    if (state == app__life_cycle__state__channel.value()) {
      return;
    }

    app__life_cycle__state__channel.value__mutation__dispatch(
      state,
    );
  ...
  @override
  widget___protocol build(_) {
    return this.widget.child__build(context);
  }
}
...
extension FlutterErrorDetails__extension on FlutterErrorDetails {
  ({string error__text, StackTrace? trace}) convert__text() {
    ...
    if (program__compilation__debug___ok) {
      buffer
        ..write(
          "\","
